...
def benchmark_ridge_strict(user_id="76561198039155404"):
    ...
    print(f"\n--- Ridge Leakage Benchmark ---")
    
    for train_idx, test_idx in kf.split(range(N)):
        # --- LEAKY ASSEMBLY (Previous Method) ---
        # Features are pre-calculated using the FULL y_dev
        K_exp_leaky = np.exp(K_full * 10.0) * lp_mask
        np.fill_diagonal(K_exp_leaky, 0.0)
        X_k_leaky = (np.sum(K_exp_leaky * y_dev_global, axis=1) / (np.sum(K_exp_leaky, axis=1) + 1e-9))
        
        G_leaky = G_full * lp_mask
        np.fill_diagonal(G_leaky, 0.0)
        X_g_leaky = (np.sum(G_leaky * y_dev_global, axis=1) / (np.sum(G_leaky, axis=1) + 1e-9))
        
        X_leaky = np.hstack([X_k_leaky.reshape(-1, 1), X_g_leaky.reshape(-1, 1), X_static])
        model_leaky = RidgeCV(alphas=[0.1, 1.0, 10.0]).fit(X_leaky[train_idx], y[train_idx])
        leaky_scores.append(model_leaky.score(X_leaky[test_idx], y[test_idx]))

        # --- STRICT ASSEMBLY (True OOS) ---
        ...
        model_strict = RidgeCV(alphas=[0.1, 1.0, 10.0]).fit(X_train, y[train_idx])
        strict_scores.append(model_strict.score(X_test, y[test_idx]))

    print(f"Leaky Ridge OOS R2:  {np.mean(leaky_scores):.4f}")
    print(f"Strict Ridge OOS R2: {np.mean(strict_scores):.4f}")

if __name__ == "__main__":
    benchmark_ridge_strict()
