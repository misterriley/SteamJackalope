...
def format_result(game, rank=1):
    ...
    output = []
    output.append("-" * 40)
    output.append(f"#{rank}: {game['name']}")
    output.append(f"Steam: {steam_link}")
    output.append(f"📅 {release_date} | ⏱️  {length_str} | 🎯 {difficulty_str}")
    ...
    if total > 0:
        ...
        output.append(f"👍 {percentage:.1f}% positive ({total:,} reviews)")
