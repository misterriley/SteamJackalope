...
def analyze_detroit_v3():
    df = pd.read_parquet(METADATA_FILE)
    aid_to_idx = {int(a): i for i, a in enumerate(df['appid'])}
    
    seed_appid = 1222140 # Detroit
    bad_appids = [12150, 371300] # Max Payne 2, Knee Deep
    ...
    print(f"Seed: {df.iloc[s_idx]['name']} (Cinematic: {is_cinematic_s}) | MIGs: {seed_migs}\n")

    # BACKGROUND POPULATION for meaningful CDF calculation
    ...
    for appid in bad_appids:
        ...
        # Calculate kernel for all test indices but only report the last one
