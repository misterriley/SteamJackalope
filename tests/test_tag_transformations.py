...
class TestTagTransformations(unittest.TestCase):
    def setUp(self):
        ...
        def test_anscombe_zero_game(self):
            ...
            vectors, _ = apply_tag_transform(test_counts, self.prior_G, test_votes, self.K, transform_type='anscombe')
        ...
        def test_clr_zero_game(self):
            ...
            vectors, _ = apply_tag_transform(test_counts, self.prior_G, test_votes, self.K, transform_type='clr')
            np.testing.assert_array_almost_equal(vectors, np.zeros_like(vectors), decimal=6)
        ...
        def test_none_zero_game(self):
            ...
            vectors, _ = apply_tag_transform(test_counts, self.prior_G, test_votes, self.K, transform_type='none')
            np.testing.assert_array_almost_equal(vectors, np.zeros_like(vectors), decimal=6)
            def test_dampening_effect(self):
                ...
                vectors_high, _ = apply_tag_transform(signal_counts_high, self.prior_G, votes_high, self.K, transform_type='none')
                vectors_low, _ = apply_tag_transform(signal_counts_low, self.prior_G, votes_low, self.K, transform_type='none')
            ...
            def test_none_zero_sum(self):
                ...
                vectors, _ = apply_tag_transform(zero_counts, self.prior_G, zero_votes, self.K, transform_type='none')
