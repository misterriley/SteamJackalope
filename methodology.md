...
- **Whitening:** To improve semantic precision, we apply [Zero-phase Component Analysis (ZCA) Whitening](https://martin-thoma.com/zca-whitening/) to the semantic vectors. Raw transformer embeddings on specialized sets of text (e.g., video game descriptions) often have high correlation between dimensions and a significant non-zero mean, which can bias searches toward common semantic themes. We apply non-centered ZCA on the data to decorrelate dimensions. This ensures that the similarity metric focuses on unique game characteristics and improves the specificity of natural language queries. The stored transformation matrix is applied to user prompts at query time to maintain alignment in the decorrelated space. 
