...
def benchmark_meta_plus_graph_elasticnet(user_id="76561198039155404"):
    ...
    print(f"\n--- Metadata + Graph ElasticNet Benchmark (Strict OOS) ---")
    
    # Grid of L1 ratios (0.01 is Ridge-heavy, 1.0 is pure Lasso)
    l1_ratios = [.01, .1, .5, .7, .9, .95, .99, 1]
    ...
    for train_idx, test_idx in kf.split(range(N)):
        ...
        # ElasticNetCV automatically finds alpha for each l1_ratio
        model = ElasticNetCV(l1_ratio=l1_ratios, cv=5, max_iter=10000).fit(X_train, y[train_idx])
        oos_scores.append(model.score(X_test, y[test_idx]))
        selected_ratios.append(model.l1_ratio_)
        selected_alphas.append(model.alpha_)

    print(f"Metadata + Graph ElasticNet OOS R2: {np.mean(oos_scores):.4f}")
    print(f"Selected Parameters per Fold:")
    for i in range(5):
        print(f" Fold {i+1}: L1 Ratio={selected_ratios[i]:.2f}, Alpha={selected_alphas[i]:.6f}")

if __name__ == "__main__":
    benchmark_meta_plus_graph_elasticnet()
