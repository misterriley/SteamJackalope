...
def run_analysis_clean():
    ...
    print("Generating CLR Tag Vectors...")
    original_use_whitening = gtv.USE_TAG_WHITENING
    gtv.USE_TAG_WHITENING = False
    temp_v = "research/temp_clr_for_mot_clean.npy"
    temp_c = "research/temp_clr_const_for_mot_clean.json"
    ...
    for _, row in df_mot.iterrows():
        ...
        long_desc = str(row['Long Description'])
        ...
        for idx in top_game_indices:
            ...
            top_games.append({'name': game['name'], 'score': float(hybrid_scores[idx])})
            
        results.append({'motivation': mot_name, 'top_tags': top_tags, 'top_games': top_games})

    print("\n" + "="*80)
    print("GAMER MOTIVATION ANALYSIS REPORT (CLEAN)")
    print("="*80)
