...
# 1. Diffused Verb Profiles
# Weighted M
MW = M * W
# Profile = MW @ S
Profiles = MW @ S
# Cosine Similarity of Profiles
profile_norms = np.linalg.norm(Profiles, axis=1) + 1e-9
Normalized_Profiles = Profiles / profile_norms[:, None]
...
def eval_game(seed_name, target_name):
    ...
    # 1. Mech Sim (Chamfer)
    active_a = get_active_verb_indices(match.iloc[0]['tags'])
    active_b = get_active_verb_indices(match_target.iloc[0]['tags'])
    
    if active_a: sim_to_a = np.max(S[:, active_a], axis=1)
    else: sim_to_a = np.zeros(V)
    
    score_a_num = M @ (sim_to_a * W)
    score_a_den = M @ W
    mask = score_a_den > 0
    ...
    # Use Sigmoid on Z-scores to preserve outliers!
    sem_z = (sem_sims - SEMANTIC_SIMILARITY_MEAN) / SEMANTIC_SIMILARITY_STD
    topic_z = (topic_sims - TOPIC_SIMILARITY_MEAN) / TOPIC_SIMILARITY_STD
    
    sem_vibe = sigmoid(sem_z, temp=2.0)
    topic_vibe = sigmoid(topic_z, temp=2.0)
    
    # Let's try simple geometric mean for vibe
    vibe_sim = softmin_blend([sem_vibe, topic_vibe], temperature=0.5)
...
print("\n--- Evaluating 20 Canonical Pairs (Cosine Mech + Sigmoid Vibe) ---")
for g1, g2 in pairs:
