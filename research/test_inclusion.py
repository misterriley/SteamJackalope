...
profile_sums = np.sum(Profiles, axis=1)
...
def eval_game(seed_name, target_name):
    ...
    # 1. Mech Sim (Fuzzy Inclusion)
    seed_profile = Profiles[seed_idx]
    seed_sum = profile_sums[seed_idx]
    
    intersection = np.sum(np.minimum(Profiles, seed_profile), axis=1)
    # Inclusion: overlap / min(len(A), len(B))
    min_sums = np.minimum(profile_sums, seed_sum)
    
    mech_sim = np.zeros(N, dtype=np.float32)
    mask = min_sums > 0
    mech_sim[mask] = intersection[mask] / min_sums[mask]
    ...
    # Scale raw similarities directly using Percentiles so they balance perfectly
    ...
    vibe_sim = softmin_blend([sem_cdf, topic_cdf], temperature=0.5)
    
    # Combine Fuzzy Inclusion Mech with Percentile Vibe
...
print("\n--- Evaluating 20 Canonical Pairs (Fuzzy Inclusion + Percentile Vibe) ---")
for g1, g2 in pairs:
