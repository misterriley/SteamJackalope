...
def run_parametric_study(steamid):
    ...
    print(f"Optimal Discovery Index: {best_idx}")
    ...
    # Range of alphas to test in RidgeCV
    alphas = np.logspace(-2, 6, 81)
    
    print(f"Starting parametric sweep from K=1 to {max_k} (step 5)...")
    k_values = list(range(1, max_k + 1, 5))
    ...
    for k in k_values:
        ...
        r2 = model.score(X_with_dummy, y_with_dummy)
        alpha = model.alpha_
        ...
        if k % 20 == 0 or k == max_k:
            print(f"  K={k:3d}: R^2={r2:.4f}, Alpha={alpha:.2f}")

    results_df = pd.DataFrame(results)
    results_df.to_csv(f"research/dna_parametric_results_{steamid}.csv", index=False)
    ...
    ax1.plot(results_df['k'], results_df['r2'], color=color, label='R^2')
    ...
    plt.title(f'DNA Parametric Study: R^2 and Alpha vs Dimensions\nUser: {steamid}')
    fig.tight_layout()
    plt.savefig(f"research/dna_parametric_plot_{steamid}.png")
    print(f"Plot saved to research/dna_parametric_plot_{steamid}.png")
    plt.show()

if __name__ == "__main__":
    steamid = "76561198039155404"
    run_parametric_study(steamid)
