...
def eval_game(seed_name, target_name):
    ...
    # 1. Mech Sim (Fuzzy Jaccard on diffused profiles)
    ...
    mask = union > 0
    mech_sim[mask] = intersection[mask] / union[mask]
    ...
    # Softmin over percentile vibes!
    vibe_sim = softmin_blend([sem_cdf, topic_cdf], temperature=0.5)
    
    # Combine Fuzzy Jaccard Mech with Percentile Vibe
...
print("\n--- Evaluating 20 Canonical Pairs (Fuzzy Jaccard Mech + Percentile Vibe) ---")
for g1, g2 in pairs:
