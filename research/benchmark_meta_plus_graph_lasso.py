...
def benchmark_meta_plus_graph_lasso(user_id="76561198039155404"):
    ...
    print(f"\n--- Metadata + Graph Lasso Benchmark (Strict OOS) ---")
    
    # Define a wide range of L1 penalties
    ...
    for train_idx, test_idx in kf.split(range(N)):
        ...
        # NOTE: Lasso often requires scaling to converge/find alpha, but since we are 
        # using the same raw features as the Ridge test for parity, we fit raw.
        # We will check if it reaches edges.
        model = LassoCV(alphas=alphas_to_test, cv=5, max_iter=10000).fit(X_train, y[train_idx])
        oos_scores.append(model.score(X_test, y[test_idx]))
        selected_alphas.append(model.alpha_)

    print(f"Metadata + Graph Lasso OOS R2: {np.mean(oos_scores):.4f}")
    print(f"Selected Alphas (Min: {alphas_to_test.min()}, Max: {alphas_to_test.max()}):")
    for i, a in enumerate(selected_alphas):
        print(f" Fold {i+1}: {a:.6f}")

if __name__ == "__main__":
    benchmark_meta_plus_graph_lasso()
