...
def analyze_mismatch():
    ...
    target_appid = 1083990 # Yume Puzzle
    
    s_idx = aid_to_idx[seed_appid]
    target_idx = aid_to_idx[target_appid]
    ...
    tag_v = np.load(TAG_VECTORS_FILE, mmap_mode='r')
    tag_n = np.load(TAG_NORMS_FILE, mmap_mode='r')
    ...
    # Pre-calculate anchor masks for Candidate
    all_needed_tags = set()
    ...
    print(f"Seed: {df.iloc[s_idx]['name']} | Target: {df.iloc[target_idx]['name']}\n")
    print(f"Seed MIGs: {seed_migs}")
    print(f"Seed Tags (Strict): {seed_tags_strict}")
    print(f"Seed Tags (Soul): {seed_tags_soul}\n")
    ...
    s_val = sim[0] if hasattr(sim, '__getitem__') else sim
    print(f"Total Sim: {float(s_val):.4f}")
    ...
    print(f"  - Combined: {float(get_val('combined')):.4f} | Tone: {float(get_val('tone')):.4f} | Diff: {float(get_val('difficulty')):.4f}")
