...
seed_game = df[df['name'] == 'Factorio'].iloc[0]
seed_idx = df.index[df['name'] == 'Factorio'][0]
...
print(f"\nTop 20 Similar Games to Factorio (Total Time: {time.time() - t0:.2f}s)")
...
print("\nSpecific Lookup:")
dyson = df[df['name'] == 'Dyson Sphere Program']
if not dyson.empty:
    row = dyson.iloc[0]
