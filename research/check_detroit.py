...
def check_detroit():
    ...
    for target_name in targets:
        try:
            ...
            # Thematic Blend (Matches current solve_user_taste.py)
            pure = (t_sim * 0.25 + s_sim * 0.25 + consensus * 0.5)
            
            # Check Vetoes
            fav_tags_str = str(df.iloc[idx1]['tags'])
            target_tags_str = str(df.iloc[idx2]['tags'])
            
            # Simple check for Narrative Veto
            NARRATIVE_TAGS = ["Story Rich", "Choices Matter", "Visual Novel", "RPG"]
            seed_narrative_score = sum([1 for t in NARRATIVE_TAGS if t in fav_tags_str])
            target_narrative_score = sum([1 for t in NARRATIVE_TAGS if t in target_tags_str])
            ...
            res.append({
                ...
                "pure": float(pure),
                "seed_narr": seed_narrative_score,
                "target_narr": target_narrative_score
            })
...
print(json.dumps(check_detroit(), indent=2))
