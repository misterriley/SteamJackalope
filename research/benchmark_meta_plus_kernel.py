...
def benchmark_meta_plus_kernel(user_id="76561198039155404"):
    ...
    # --- RAW COMPONENTS (Needed for Kernel) ---
    ...
    print(f"\n--- Metadata + Kernel Benchmark (Strict OOS) ---")
    
    for train_idx, test_idx in kf.split(range(N)):
        # KERNEL FOR TRAIN set using ONLY OTHER TRAIN neighbors
        K_train_sub = K_full[np.ix_(train_idx, train_idx)]
        lp_train = lp_mask[np.ix_(train_idx, train_idx)]
        K_exp_train = np.exp(K_train_sub * 10.0) * lp_train
        ...
        X_train = np.hstack([X_k_train.reshape(-1, 1), X_static[train_idx]])
        
        # KERNEL FOR TEST set using ONLY TRAIN neighbors
        K_test_sub = K_full[np.ix_(test_idx, train_idx)]
        lp_test = lp_mask[np.ix_(test_idx, train_idx)]
        K_exp_test = np.exp(K_test_sub * 10.0) * lp_test
        X_k_test = (np.sum(K_exp_test * y_dev_global[train_idx], axis=1) / (np.sum(K_exp_test, axis=1) + 1e-9))
        X_test = np.hstack([X_k_test.reshape(-1, 1), X_static[test_idx]])
        ...
        lasso = LassoCV(cv=5, max_iter=10000).fit(X_train, y[train_idx])
