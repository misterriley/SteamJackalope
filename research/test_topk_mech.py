...
# Precompute game representations
print("Precomputing representations...")
game_verbs = []
for i, tag_str in enumerate(df['tags']):
    active_idx = get_active_verb_indices(tag_str)
    if active_idx:
        # Sort by weight descending
        active_idx = sorted(active_idx, key=lambda x: W[x], reverse=True)
        # Keep top 4
        active_idx = active_idx[:4]
    game_verbs.append(active_idx)

# Matrix M_top for fast A->B
M_top = np.zeros((N, V), dtype=np.float32)
for i, idxs in enumerate(game_verbs):
    if idxs:
        M_top[i, idxs] = 1.0
...
def eval_game(seed_name, target_name):
    ...
    # 1. Mech Sim (Top-4 Chamfer)
    active_a = game_verbs[seed_idx]
    
    if active_a: sim_to_a = np.max(S[:, active_a], axis=1)
    else: sim_to_a = np.zeros(V)
    
    score_a_num = M_top @ (sim_to_a * W)
    score_a_den = M_top @ W
    mask = score_a_den > 0
    ...
    if active_a:
        for vb in active_a:
            w = W[vb]
            max_sim_b = np.max(M_top * S[vb, :], axis=1)
...
print("\n--- Evaluating 20 Canonical Pairs (Top-4 Chamfer Mech + Percentile Vibe) ---")
for g1, g2 in pairs:
