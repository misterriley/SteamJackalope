...
def main():
    ...
    print(f"Matched {len(train_targets_df)} unique games for training.")
    ...
    X_semantics = np.load("data/production/embeddings_desc.npy")
    
    X_all = np.hstack([X_tags, X_topics, X_semantics])
    feature_names = unique_tags + [f"topic_{i}" for i in range(X_topics.shape[1])] + [f"sem_{i}" for i in range(X_semantics.shape[1])]
    ...
    print(f"Training Lasso model with 20-fold CV on {len(y_train)} samples...")
    ...
    # Extract Mean CV MSE (Best Alpha)
    ...
    print("\n" + "="*40)
    print("   OUT-OF-SAMPLE PERFORMANCE RESULTS")
    print("="*40)
    print(f"Optimal Alpha: {model.alpha_:.6f}")
    print(f"Non-zero coefficients: {len(model.coef_[model.coef_ != 0])} / {len(model.coef_)}")
    print("-"*40)
    print(f"Training R^2: {train_r2:.4f}")
    print(f"Training MSE: {train_mse:.4f}")
    print("-"*40)
    print(f"Mean CV R^2:  {mean_cv_r2:.4f} (from 20-fold CV on train set)")
    print(f"Mean CV MSE:  {mean_cv_mse:.4f}")
    print("-"*40)
    print(f"Test R^2:     {test_r2:.4f} (TRULY UNSEEN DATA)")
    print(f"Test MSE:     {test_mse:.4f}")
    print(f"Test RMSE:    {np.sqrt(test_mse):.4f} (Difficulty points)")
    print("="*40)

    # Sanity check: Correlations of top predictors
